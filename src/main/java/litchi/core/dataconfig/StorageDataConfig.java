 ...
 * 		   2018-07-31
 */
public class StorageDataConfig implements DataConfig {
...
//	 * 配置服务管理
//	 */
//	private Map<Class<? extends ConfigAdapter>, Collection<ConfigService>> configServiceMaps = new HashMap<>();

	private Set<ConfigService> configServicesList = new HashSet<>();

	public StorageDataConfig(Litchi litchi) {
	...
	@Override
	public boolean flush2NewConfig(String fileName, String text) {
		return false;
    }
...
//        collection.forEach(clazz -> {
//        	Collection<ConfigService> configServices  = configServiceMaps.get(clazz);
...
//        });
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("load dataconfig file list:");
				...
				try {
					...
					for (Field field : declaredFields) {
						if (field.isAnnotationPresent(IndexPK.class)) {
							...
							if (annoFieldName != null && StringUtils.isNotBlank(annoFieldName.indexName())) {
								indexFields.add(field);
							}
						}
					}
...
//		List<Class<? extends ConfigAdapter>> classList = new ArrayList<>();
//		configService.listeningConfigs(classList);
//		for (Class<? extends ConfigAdapter> clazz : classList) {
//			Collection<ConfigService> collection = configServiceMaps.get(clazz);
//			if (collection == null) {
//				collection = new ArrayList<>();
//				configServiceMaps.put(clazz, collection);
//			}
//			collection.add(configService);
//		}
	}
